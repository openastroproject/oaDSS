...
#pragma omp parallel for schedule(static, 100) default(none) shared(height,progress,nrProcessors,width) reduction(+: nrHotPixels, nrColdPixels) if(nrProcessors > 1)
...
#pragma omp parallel for schedule(guided, 100) default(none) shared(progress,nrProcessors) if(nrProcessors > 1)
