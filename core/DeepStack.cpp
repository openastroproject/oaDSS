...
#pragma omp parallel default(none) firstprivate(maxValue) shared(fMax, redPixels, greenPixels, bluePixels,height,width,scalingFactor) if(nrEnabledThreads - 1)
	{
...
#pragma omp critical(OrigHistoCalcOmpCrit)
	...
	if (!m_StackedBitmap.IsMonochrome())
	{
#pragma omp parallel sections default(none) shared(Histo, redPixels, greenPixels, bluePixels,scalingFactor) if(nrEnabledThreads - 1)
		{
#pragma omp section
