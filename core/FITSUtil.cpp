...
#pragma omp parallel default(none) shared(fMin, fMax,nElements,doubleBuff) firstprivate(localMin, localMax) if(nrProcessors > 1)
			{
...
#pragma omp parallel for default(none) shared(stop,colours,doubleBuff,greenOffset,blueOffset,normalizeFloatValue,result,rowProgress) schedule(guided, 50) if(nrProcessors > 1)
