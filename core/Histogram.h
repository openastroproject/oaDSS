...
namespace DSS
{
	...
	public:
		...
		double	Adjust(double fValue) const
		{
			...
			if (fValue < minimum)
				fResult = originalMinimum + fValue / std::max(1.0, minimum - originalMinimum) * (usedMinimum - originalMinimum);
			else if (fValue > maximum)
				fResult = originalMaximum - (fValue - maximum) / std::max(1.0, originalMaximum - maximum) * (originalMaximum - usedMaximum);
			else
				fResult = usedMinimum + AdjustValue((fValue - minimum) / std::max(1.0, (maximum - minimum))) * (usedMaximum - usedMinimum);
	...
	public:
		...
		void	AddValue(double fValue, int lNrValues = 1)
		{
			...
			if (lNrStep < values.size())
			{
				...
				intMax = std::max(intMax, static_cast<uint32_t>(values[lNrStep]));

				maximum = std::max(maximum, fValue);
				if (minimum < 0)
					minimum = fValue;
				else
					minimum = std::min(minimum, fValue);
			};
		};
