...
namespace DSS
{
	...
	void RecommendedSettings::fillWithRecommendedSettings()
	{
		...
		if (pStackingTasks && pStackingTasks->GetNrLightFrames())
		{
			...
			if (std::max(pStackingTasks->GetNrDarkFrames(), pStackingTasks->GetNrDarkFlatFrames()))
				AddDarkMethod(vRecommendations, std::max(pStackingTasks->GetNrDarkFrames(), pStackingTasks->GetNrDarkFlatFrames()));
